...
| ein Zigbee- oder Matter-Gerät anlernen willst | in der App unter Einstellungen → Verbindungen, Abschnitt «Gerät hinzufügen» (Punkt 632) – Zigbee öffnet das Netz für vier Minuten und meldet, wer anklopft (`integrations/zigbee2mqtt.py`, `anlernen_starten`), Matter nimmt den Code vom Aufkleber (`integrations/matter.py`, `pair`); Routen in `api/routes/verbindungen.py`, App in `components/GeraetAnlernen.tsx` + `lib/anlernen.ts`. Das neue Gerät hat keinen Raum und steht danach unter Geräte → «Noch einzurichten» |
...
| eine PlayStation koppeln willst | in der App unter Einstellungen → Verbindungen, Abschnitt «PlayStation» (Punkt 643) – zuerst die PSN-Anmeldung (Adresse der Seite danach einfügen), dann der achtstellige Code von der Konsole (Einstellungen → System → Remote Play → «Gerät verbinden», die Konsole muss dabei an sein). Hub in `integrations/playstation.py` (DDP für Zustand und Wecken ohne Bibliothek, `pyremoteplay` für Ruhemodus und Tasten in einer kurzlebigen Sitzung) + `api/routes/playstation.py`; Konto in `playstation-token.json`, Registrierung in `playstation-profile.json`, beides neben der Datendatei. Übersicht und Grenzen in `docs/playstation.md`; `python -m homepilot.integrations.playstation <host>` zeigt roh, was die Konsole antwortet |
...
| wissen willst, warum die Alarmanlage erst spät von selbst scharf wurde | `docker exec homepilot-hub python -m homepilot.anwesenheitscheck` – je Person Zustand, Quelle, letzte Meldung und Akku, das Kommen und Gehen der letzten 24 Stunden und die Rechnung dahinter: seit wann «alle weg» gilt, an wem es hing und wann die Kopplung frühestens schalten durfte. Die Anlage wartet auf den **Letzten**, und sie rechnet mit der Meldung, nicht mit dem Weggang |
...
| dich fragst, warum nach einem Stromausfall alles Licht brennt | `hub/homepilot/core/stromrueckkehr.py` (erkennt den Kaltstart) + der Auslöser «Nach Stromausfall» in `core/automation.py` – was dann gilt, steht in einem Ablauf; den Blitz selbst verhindert die Einstellung am Gerät, und die stellt der Hub seit Punkt 630 (`set_power_on` in `zigbee2mqtt.py`, `hue.py`, `homematic.py`; App: Anpassen-Blatt und unter System der Knopf «Alle Lampen auf ‹wie vorher›», `components/StromausfallCard.tsx`) |
