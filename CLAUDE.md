...
| einen Android TV koppeln willst | in der App auf der Fernsehkachel: «Fernseher koppeln», zwischen «App wechseln» und «Einschlafen» (der Fernseher muss dabei an sein) – Code in `app/src/components/TvKopplung.tsx` + `lib/fernsehkopplung.ts`, Hub in `integrations/androidtv.py` (`pair_start`/`pair_finish`) + `api/routes/androidtv.py` |
