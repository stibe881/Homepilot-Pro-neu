...
Zähler in `core/metrics.py`, neben der bereits vorhandenen
Bildschirm-Nutzung je Gerät. Aufwand: klein · Hub.
...
**672. Kurzhilfe automatisch beim ersten Öffnen eines Moduls.** Die
Seitenhilfe (`lib/seitenhilfe.ts`) gibt es auf Abruf - fehlt: das
automatische Zeigen beim allerersten Besuch, wie bei der Einführung.
Aufwand: klein · App.
...
**681. ESLint-Regel gegen literale Schriftgrössen** ausserhalb der
Skala (`lib/schrift.ts`), analog zur padding-Regel aus Punkt 441.
Aufwand: klein · App.
...
**695. Duplikaterkennung direkt beim Scannen einer Karte** (Code-
Vergleich im Scan-Moment), nicht erst beim Speichern. Aufwand: klein ·
App+Hub.
...
Komfort, Energie) statt nur alphabetisch. Aufwand: klein · Hub+App.
