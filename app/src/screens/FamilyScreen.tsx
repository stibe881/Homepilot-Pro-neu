...
export function FamilyScreen({
  ...
  // Die sechs Module, die für sich stehen (Punkt 423 der Werkbank) -
  // sie liegen in family/module.tsx. Der Rahmen ist ein Objekt: Wer das
  // nächste Modul herauszieht, erweitert eine Zeile statt sieben
  // Aufrufe, und genau daran scheitern solche Schnitte sonst auf halbem
  // Weg.
  const modulrahmen = { data, styles, colors, goBack, add, update, remove };
  if (view === 'emergency') return <Notfallblatt {...modulrahmen} />;
  if (view === 'medications')
    return <Medikamente {...modulrahmen} members={members} ich={currentUser?.name} />;
  if (view === 'packlists') return <Packlisten {...modulrahmen} />;
  if (view === 'reminders')
